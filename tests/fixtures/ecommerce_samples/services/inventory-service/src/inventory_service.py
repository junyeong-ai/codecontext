...
class InventoryService:
    ...
    def check_stock(self, product_id: UUID, quantity: int) -> bool:
        ...
        self.logger.info(
            f"Stock check result: {available} " f"(total: {total_stock}, required: {quantity})"
        )
    ...
    def reserve_stock(self, order_items: list["OrderItem"]) -> "Reservation":
        ...
        for item in order_items:
            ...
            self.logger.info(
                f"Reserved {item.quantity} units of {item.product_id} "
                f"at warehouse {warehouse.id}"
            )
        ...
        for warehouse in self.warehouses:
            for stock in warehouse.get_all_stocks():
                if stock.available_quantity <= stock.reorder_point:
                    ...
                    self.logger.info(
                        f"Reorder needed: {stock.product_id} " f"at warehouse {warehouse.id}"
                    )
