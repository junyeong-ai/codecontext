...
def mock_config():
    ...
    config.indexing.parallel_workers = 4  # Not used when parallel_enabled=False
    config.indexing.parallel_enabled = False  # Disable parallel for tests
    ...
    config.embeddings.provider = "huggingface"  # Must be string for provider check
...
class TestSyncErrorHandling:
    ...
    def test_continues_on_file_extraction_error(
        ...
        # Mock _stream_embeddings to avoid multiprocessing issues with Mock objects
        async def mock_stream_embeddings(objects, show_progress=True):
            """Mock streaming embeddings - just return objects with dummy embeddings."""
            for obj in objects:
        ...
        with (
            ...
            patch.object(sync, "_stream_embeddings", side_effect=mock_stream_embeddings),
        ):
            state = asyncio.run(sync.index(tmp_path, show_progress=False))
