...
**Implementations**:
- [ChromaDBVectorStore](../../packages/codecontext-storage-chromadb/src/codecontext_storage_chromadb/provider.py)
...
# Create provider
config = StorageConfig(provider="chromadb")
provider = StorageProviderFactory.create("chromadb", config)
provider.initialize()
