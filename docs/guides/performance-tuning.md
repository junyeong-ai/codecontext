...
**High-performance**:
```bash
# Start ChromaDB with more resources
...
storage:
  chromadb:
    host: localhost
    port: 8000
    timeout_seconds: 30  # Increase for large queries
```

### 3. Index Optimization

ChromaDB automatically optimizes indexes. No manual tuning required.
...
3. **Storage Operations** (20%+ of time)
   - **Solution**: Increase `batch_size`
   - **Solution**: Use SSD for ChromaDB storage
