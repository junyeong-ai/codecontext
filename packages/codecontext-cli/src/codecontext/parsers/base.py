...
class BaseCodeParser(CodeParser):
    ...
    def __init__(
        ...
        self.cast_chunker = CASTChunker(self.chunking_config)
    ...
    def extract_cast_chunks(self, file_path: Path, source_code: str) -> list[CASTChunk]:
        """
        Extract cAST chunks from a source file using structure-preserving chunking.

        This method uses the CASTChunker to create semantic chunks that preserve
        code structure and include necessary context (imports, parent definitions).
        ...
        Returns:
            List of extracted CASTChunk objects

        Raises:
            ParserError: If extraction fails
        """
        # Use language name mapping
        language_map = {
        ...
        language_str = language_map.get(self.language, "unknown")
        return self.cast_chunker.chunk_file(file_path, source_code, language_str)
    ...
    def _extract_enums_generic(
        ...
        for enum_node in enum_candidates:
            ...
            nl_desc = self._generate_enum_nl(name=name, values_count=len(members))
