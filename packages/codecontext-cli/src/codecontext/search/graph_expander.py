...
class GraphExpander:
    ...
    # Relationship type priorities (edge weights for PPR)
    EDGE_WEIGHTS: dict[RelationType, float] = {
        # Document-Code links (highest priority)
        RelationType.DOCUMENTS: 1.0,
        RelationType.DOCUMENTED_BY: 1.0,
        ...
        # Other relationships (lower priority)
    ...
    def __init__(
        ...
        self.enabled = getattr(config.search, "enable_graph_expansion", True)
        self.max_hops = getattr(config.search, "graph_max_hops", 1)
        self.ppr_threshold = getattr(config.search, "graph_ppr_threshold", 0.3)
        self.score_weight = getattr(config.search, "graph_score_weight", 0.3)
        ...
        try:
            ...
            for code_obj in code_objects:
                entities_map[code_obj.deterministic_id] = SearchResult(
                    ...
                    nl_description=getattr(code_obj, "nl_description", ""),
                    language=(
