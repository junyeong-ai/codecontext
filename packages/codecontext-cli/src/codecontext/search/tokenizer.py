...
class CodeTokenizer:
    ...
    def tokenize_document(
        ...
        Tokens from higher-weighted fields are repeated to boost relevance.
        For example, with weight=3.0, tokens appear 3 times in the output.

        Args:
            doc: Document with text fields
            field_weights: Mapping of field names to weights (e.g., {"name": 3.0})

        Returns:
            List of tokens with field-weighted repetition

        Examples:
            >>> doc = {"name": "getUserById", "content": "Fetch user data"}
            >>> weights = {"name": 3.0, "content": 1.0}
            >>> tokens = CodeTokenizer.tokenize_document(doc, weights)
            >>> tokens.count("get")  # "get" appears 3x from name
            3
        """
        all_tokens = []
        ...
        for field, weight in field_weights.items():
            # Check if field exists and has non-None value
            if field in doc and doc[field] is not None:
                text = str(doc[field])
                # Skip if text is empty after conversion
                if text:
                    tokens = CodeTokenizer.tokenize_text(text)

                    # Repeat tokens based on weight (simple boosting)
