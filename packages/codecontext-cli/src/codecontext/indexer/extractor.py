...
class Extractor:
    ...
    def _init_patterns(self) -> None:
        ...
        Supports: Python, JavaScript, TypeScript, Java, Kotlin (CALLS, REFERENCES, INHERITS, IMPORTS)
        """
        ...
        # INHERITS patterns - class inheritance
    ...
    def _select_best_target(
        ...
        if not (hasattr(node, "start_point") and source_obj.location):
            return candidates[0]

        ref_line = node.start_point[0] + 1
        source_line = source_obj.location.start_line

        best_obj = candidates[0]
        min_distance = abs(source_line - ref_line) if candidates[0].location else float("inf")

        for obj in candidates[1:]:
            if obj.location:
                distance = abs(obj.location.start_line - ref_line)
    ...
    def _select_best_context(self, parent: Node, candidates: list[CodeObject]) -> CodeObject | None:
        ...
        if hasattr(parent, "start_point") and hasattr(parent, "end_point"):
            ...
            for obj in candidates:
                if obj.location and obj.location.start_line == parent_start_line:
                    return obj
