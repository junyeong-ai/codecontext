...
CodeContext supports indexing and searching multiple projects independently with isolated ChromaDB collections.

## Features

- **Auto-detection**: Project IDs automatically detected from Git remote URLs
- **Isolation**: Each project has separate ChromaDB collections (no cross-contamination)
...
Each project gets 4 ChromaDB collections:

```
codecontext_index_{project_id}_code_objects
codecontext_index_{project_id}_documents
codecontext_index_{project_id}_relationships
codecontext_index_{project_id}_state
```

Example for project `my-app`:
```
codecontext_index_my-app_code_objects
codecontext_index_my-app_documents
codecontext_index_my-app_relationships
codecontext_index_my-app_state
```
...
| Delete project | ~200ms (4 collections) |

### Storage Requirements

Per project overhead:
- **Metadata**: ~1KB per project (IndexState)
- **Collections**: 4 ChromaDB collections
- **Indexes**: Separate HNSW indexes per project
...
1. **Check current collections**:
   ```bash
   # Via chroma-cli.sh or ChromaDB API
   # Look for: codecontext_index_code_objects, etc.
   ```

2. **Rename collections** (manual):
   ```python
   # Note: ChromaDB doesn't support collection renaming
   # You must re-index
