...
1. **ChromaDB Server**:
   ...
   ./scripts/chroma-cli.sh status  # verify running
   ```

2. **E-commerce Sample Dataset**: `tests/fixtures/ecommerce_samples/`
...
**Issue: 0% Pass Rate**
- **Cause**: Wrong dataset indexed or ChromaDB has stale data
- **Fix**:
  ```bash
  ./scripts/clean-chromadb.py  # Clear all indexed data
  cd tests/fixtures/ecommerce_samples
